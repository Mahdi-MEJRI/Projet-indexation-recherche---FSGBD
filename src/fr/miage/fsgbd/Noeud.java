...
public class Noeud<Type> implements java.io.Serializable {
    ...
    // Retourne vrai s'il le noeud s'agit d'une feuille ou non
    public boolean estFeuille() {
        return this.fils.size() == 0;
    }

    // Retourne la premiere feuille du noeud
    public Noeud<Type> getPremFeui() {
        if (this.estFeuille()) return this;
        else return this.fils.get(0).getPremFeui();
    }

    // Retourne la feuille suivante
    public Noeud<Type> getFeuiSuiv() {
        Noeud<Type> racine = this;
        if (this.getNoeudSuivant() != null) return this.getNoeudSuivant();
        else {
            // On remonte et on s'arrete avant le racine
            while (racine.parent.parent != null) {
                if (racine.parent.getNoeudSuivant() != null) return racine.parent.getNoeudSuivant().getPremFeui();
                else racine = racine.parent;
            }
        }
        return null;
    }

    // Si le noeud est une feuille, la methode l'affiche ainsi que toutes feuilles suivantes séquenciellement (message
    // d'erreur sinon)
    // Si on lui applique sur la premiere feuille, la methode affiche toute les feuilles de l'arbre
    public void affichSeqFeuilles() {
        if (this.estFeuille()) {
            StringBuilder feuille = new StringBuilder();
            for (Type valeur : this.keys) {
                feuille.append(valeur.toString()).append(" ");
            }
            System.out.println(feuille);
            if (this.getFeuiSuiv() != null)
                this.getFeuiSuiv().affichSeqFeuilles();
        }
        else
            System.out.println("Le noeud qui appelle la méthode doit être une feuille !");
    }
